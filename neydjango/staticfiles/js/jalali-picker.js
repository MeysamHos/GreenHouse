...
(function () {
  ...
  function jalaaliToGregorian(jy, jm, jd) {
    var sal_a, gy, gm, gd, jdo, gdo, j_remainder, g_remainder;
    jy += 1595;
    jdo = -355779 + 365 * jy + Math.floor(jy / 33) * 8 +
          Math.floor(((jy % 33) + 3) / 4);
    gy = 400 * Math.floor(jdo / 146097);
    jdo %= 146097;
    if (jdo > 36524) {
      gy += 100 * Math.floor(--jdo / 36524);
      jdo %= 36524;
      if (jdo >= 365) jdo++;
    }
    gy += 4 * Math.floor(jdo / 1461);
    jdo %= 1461;
    if (jdo > 364) {
      gy += Math.floor((jdo - 1) / 365);
      jdo = (jdo - 1) % 365;
    }
    gdo = jdo + 1;
    var sal_b = [0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
    if ((gy % 4 === 0 && gy % 100 !== 0) || gy % 400 === 0) sal_b[2] = 29;
    gm = 0;
    for (var i = 1; i <= 12; i++) {
      if (gdo <= sal_b[i]) { gm = i; gd = gdo; break; }
      gdo -= sal_b[i];
    }
